import Autobase from 'autobase'
import Hyperbee from 'hyperbee'
import Hyperblobs from 'hyperblobs'
import b4a from 'b4a'

class PearSyncAutobase {
  constructor (store, blobs, bootstrap = null) {
    this.store = store
    this.blobs = blobs
    this.base = null
    this.localWriter = null
    this.bootstrap = bootstrap
    this._ready = false
  }

  async ready () {
    if (this._ready) return

    const bootstrap = this.bootstrap ? this.bootstrap[0] : null

    this.base = new Autobase(this.store, bootstrap, {
      open: this._open.bind(this),
      apply: this._applyBatch.bind(this),
      valueEncoding: 'json'
    })

    await this.base.ready()
    this.localWriter = this.base.local.key

    this._ready = true
  }

  _open (store) {
    return store.get('view', { valueEncoding: 'binary' })
  }

  async _applyBatch (batch, view, base) {
    const bee = new Hyperbee(view, {
      keyEncoding: 'utf-8',
      valueEncoding: 'json'
    })

    for (const node of batch) {
      const op = node.value
      if (!op) continue

      // Handle writer management first
      if (op.type === 'add-writer' || op.add) {
        const keyHex = op.writerKey || op.add
        const key = b4a.from(keyHex, 'hex')
        console.log(`[apply] Processing add-writer: ${keyHex.slice(0, 16)}...`)
        await base.addWriter(key, { indexer: true })
        console.log(`[apply] Added writer successfully`)
        // Continue to next - don't write admin ops to Hyperbee
        continue
      }
      
      if (op.type === 'remove-writer') {
        const key = b4a.from(op.writerKey, 'hex')
        const nodeWriterKey = b4a.toString(node.writer.key, 'hex')
        const targetKey = b4a.toString(key, 'hex')
        if (nodeWriterKey === targetKey) {
          await base.removeWriter(key)
        }
        continue
      }
      
      // Handle data operations via Hyperbee
      if (op.type === 'put') {
        await bee.put(op.key, op.value)
      } else if (op.type === 'del') {
        await bee.del(op.key)
      }
    }
  }

  async append (op) {
    await this.base.append(op)
  }

  async addWriter (writerKey) {
    // Writers must be added through the apply function via messages
    // This ensures Autobase's internal state is correctly updated
    const key = typeof writerKey === 'string' ? writerKey : b4a.toString(writerKey, 'hex')
    await this.append({
      type: 'add-writer',
      writerKey: key
    })
    // Wait for the apply function to process the message
    await this.base.view.update()
  }

  async removeWriter (writerKey) {
    const key = typeof writerKey === 'string' ? b4a.from(writerKey, 'hex') : writerKey
    const isSelf = b4a.toString(key, 'hex') === b4a.toString(this.localWriter, 'hex')

    if (isSelf) {
      const keyHex = b4a.toString(key, 'hex')
      for (const [k, writer] of this.base.activeWriters.map) {
        if (k === keyHex) {
          this.base.activeWriters.delete(writer.core)
          break
        }
      }
      await this.base.view.update()
    }
  }

  async putBlob (content) {
    if (!this.blobs) {
      throw new Error('Blobs not configured - use embedded content instead')
    }
    const descriptor = await this.blobs.put(content)
    return descriptor
  }

  async getBlob (descriptor) {
    if (!this.blobs) {
      throw new Error('Blobs not configured - use embedded content instead')
    }
    return await this.blobs.get(descriptor)
  }

  async list () {
    const bee = new Hyperbee(this.base.view, {
      keyEncoding: 'utf-8',
      valueEncoding: 'json'
    })

    const entries = []
    for await (const entry of bee.createReadStream()) {
      entries.push(entry)
    }
    return entries
  }

  async close () {
    await this.base.close()
  }
}

export { PearSyncAutobase }
