...
class PearSyncAutobase {
  ...
  async _applyBatch (batch, view, base) {
    ...
    for (const node of batch) {
      ...
      } else if (op.type === 'add-writer' || op.add) {
        // Handle both formats: { type: 'add-writer', writerKey } and { add: key }
        const keyHex = op.writerKey || op.add
        const key = b4a.from(keyHex, 'hex')
        // Add writer - use default indexer: true like the Autobase example
